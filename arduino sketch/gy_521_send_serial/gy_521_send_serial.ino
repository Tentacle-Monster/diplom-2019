...
void calibrate_sensors() {
  ...
  //mySerial.println("Starting Calibration");
  ...
  //mySerial.println("Finishing Calibration");
}
...
{      
  ...
  */
  ...
  /*
  mySerial.print(F("WHO_AM_I : "));
  mySerial.print(c,HEX);
  mySerial.print(F(", error = "));
  mySerial.println(error,DEC);
  */
...
/*
  mySerial.print(F("Read accel, temp and gyro, error = "));
  mySerial.println(error,DEC);
  

  // Print the raw acceleration values
  mySerial.print(F("accel x,y,z: "));
  mySerial.print(accel_t_gyro.value.x_accel, DEC);
  mySerial.print(F(", "));
  mySerial.print(accel_t_gyro.value.y_accel, DEC);
  mySerial.print(F(", "));
  mySerial.print(accel_t_gyro.value.z_accel, DEC);
  mySerial.println(F(""));
*/ 
...
/*  
  ...
  mySerial.print(dT, 3);
  mySerial.print(F(" degrees Celsius"));
  mySerial.println(F(""));
  

  // Print the raw gyro values.
  mySerial.print(F("raw gyro x,y,z : "));
  mySerial.print(accel_t_gyro.value.x_gyro, DEC);
  mySerial.print(F(", "));
  mySerial.print(accel_t_gyro.value.y_gyro, DEC);
  mySerial.print(F(", "));
  mySerial.print(accel_t_gyro.value.z_gyro, DEC);
  mySerial.print(F(", "));
  mySerial.println(F(""));
*/
 ...
 // float dir = accel_z/fabs(accel_z);
//  float accel_vector_length = sqrt(pow(accel_x,2) + pow(accel_y,2) + pow(accel_z,2));
  float accel_angle_y = atan2(-1*accel_x  , gyro_x/fabs(gyro_x)* sqrt(pow(accel_y,2) + pow(accel_z,2)))*RADIANS_TO_DEGREES;
  float accel_angle_x = atan(accel_y    / gyro_y/fabs(gyro_y)*sqrt(pow(accel_x,2) + pow(accel_z,2)))*RADIANS_TO_DEGREES;
 ...
 // last_z_angle= ((int)last_z_angle);
  ...
  /*
  // Send the data to the mySerial port
  ...
  mySerial.print(unfiltered_gyro_angle_x, 2);        //Gyroscope angle
  mySerial.print(F(","));
  mySerial.print(unfiltered_gyro_angle_y, 2);
  mySerial.print(F(","));
  mySerial.print(unfiltered_gyro_angle_z, 2);
  mySerial.print(F("#FIL:"));             //Filtered angle
  */
   /*
...
*/
   ...
   pocket[0]=pocket[7]=0xFF;
        pocket[1]=pocket[6]=0xAA;

        FLOATUNION_t myFloat;
        myFloat.number = angle_x; // Assign a number to the float
        for (int i=0; i<4; i++)
        {
        pocket[2+i]=myFloat.bytes[i];
        }
       ...
       // mySerial.println("");
        ...
        mySerial.println((char*)pocket);
          ...
          pocket[0]=pocket[7]=0xFF;
        pocket[1]=pocket[6]=0xBB;

       
        myFloat.number = angle_y; // Assign a number to the float
        for (int i=0; i<4; i++)
        {
        pocket[2+i]=myFloat.bytes[i];
        }
        ...
        pocket[1]=pocket[6]=0xCC;
        
        myFloat.number = angle_z; // Assign a number to the float
        for (int i=0; i<4; i++)
        {
        pocket[2+i]=myFloat.bytes[i];
        }
  ...
  // Delay so we don't swamp the mySerial port
  delay(5);
}
